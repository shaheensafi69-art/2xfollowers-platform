...
import { 
  ...
  Monitor, FileText, Wallet, Globe, ChevronDown, ArrowRight, CheckCircle2, CreditCard, Coins
} from 'lucide-react';
...
const PLATFORM_UI: { [key: string]: { icon: any, color: string, bg: string } } = {
  ...
  'HMA': { icon: ShieldCheck, color: 'text-red-600', bg: 'bg-red-600' },
...
export default function NewOrderPage() {
  ...
  const calculateTotal = () => {
    ...
    const total = isFixedProduct ? selectedService.price : (selectedService.price * quantity / 1000);
    return total < 0.01 && total > 0 ? 0.01 : total;
  };
  ...
  const handlePayment = async (gatewayId: string, status: string) => {
    ...
    if (!selectedService || !link || total <= 0) {
        ...
        <div className="lg:col-span-3 bg-white p-6 sm:p-10 rounded-[3rem] border border-slate-100 shadow-xl space-y-8">
              ...
              <div className="absolute top-full left-0 right-0 mt-3 bg-white border border-slate-100 rounded-[2rem] shadow-2xl z-[100] max-h-[350px] overflow-y-auto p-3 grid grid-cols-1 sm:grid-cols-2 gap-2 animate-in fade-in zoom-in-95">
                {CATEGORY_MAP.map((cat) => (
                  <button key={cat} onClick={() => { 
                    ...
                    <div className={`${PLATFORM_UI[cat].bg} p-2.5 rounded-xl`}>{(() => { const Icon = PLATFORM_UI[cat].icon; return <Icon className={PLATFORM_UI[cat].color} size={18} />; })()}</div>
          ...
          {!showGateways ? (
            <button 
              ...
              className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black text-xl hover:bg-emerald-600 transition-all flex justify-center items-center gap-3 shadow-xl active:scale-95"
            >
              CHOOSE PAYMENT <ArrowRight size={20} />
            </button>
          ) : (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {PAYMENT_GATEWAYS.map((gt) => (
                  ...
                  >
                    ...
                    {gt.status === 'soon' && (
                      <span className="absolute -top-1 -right-1 text-[6px] font-black text-white bg-slate-400 px-2 py-0.5 rounded-full uppercase">Soon</span>
                    )}
                  </button>
                ))}
              </div>
              <button onClick={() => setShowGateways(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:text-slate-900 text-center">← EDIT ORDER DETAILS</button>
            </div>
          )}
        </div>
          ...
          <div className="bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl sticky top-28 border border-white/5 overflow-hidden">
            ...
            {selectedService ? (
              ...
              <div className="text-center py-16 opacity-30">
                ...
                <p className="font-black uppercase italic tracking-[0.2em] text-sm">Waiting Selection</p>
              </div>
            )}
          </div>
        </div>
